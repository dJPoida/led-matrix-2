...
type IosScrollFixState = Record<string, unknown>;
...
/**
 * Performs some manual tweaks to the way the interface scrolls on iOS devices
 *
 * TODO: This is a legacy component and needs to be upgraded to a functional component
 */
  ...
  private iosScrollFixTouchMove = (e: TouchEvent) => {
    ...
    if (this.divRef.current) {
      ...
      if (scrollTop === 0 && direction === 'up') {
        ...
        scrollTop >= this.divRef.current.scrollHeight - this.divRef.current.clientHeight &&
        direction === 'down'
      ) {
  ...
  private iosScrollFixWithBounce = (e: Event) => {
    if (this.divRef.current) {
      ...
      if (scrollTop <= 0 && scrollTop >= 0 - BOUNCE_SCROLL_PIXEL_FIX_RANGE && this.lastY < 0) {
        ...
        scrollTop <= maxScrollTop + BOUNCE_SCROLL_PIXEL_FIX_RANGE &&
        this.lastY > maxScrollTop
      ) {
  ...
  private addIosScrollFix = () => {
    if (this.divRef.current && platformInformation().isIOS) {
      if (!USE_BOUNCE_SCROLL) {
        ...
        this.divRef.current.addEventListener('scroll', this.IosScrollFixScrollRequestAnimationFrame, {
          passive: true,
          capture: true,
        });
      } else {
